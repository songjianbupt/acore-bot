...
*/
    struct spellbreaker_botAI : public bot_ai
    {
        ...
        void ApplyClassEffectMods(WorldObject const* /*wtarget*/, SpellInfo const* spellInfo, uint8 effIndex, float& value) const override
        {
            ...
            if (baseId == FEEDBACK_EFFECT && effIndex == EFFECT_0)
                value += me->SpellBaseDamageBonusDone(SPELL_SCHOOL_MASK_MAGIC) * 0.5f * me->GetVictim()->CalculateDefaultCoefficient(spellInfo, SPELL_DIRECT_DAMAGE) * me->GetVictim()->CalculateLevelPenalty(spellInfo);
    ...
    private:
        ...
        void ProcessSpellsteal(Unit* target)
        {
            ...
            //LOG_ERROR("entities.unit", "ProcessSpellsteal: on %s, fr=%u", target->GetName().c_str(), uint32(isFriend));
