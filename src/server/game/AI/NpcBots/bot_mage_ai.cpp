...
*/
    struct mage_botAI : public bot_ai
    {
        ...
        void ApplyClassDamageMultiplierSpell(int32& damage, SpellNonMeleeDamage& damageinfo, SpellInfo const* spellInfo, WeaponAttackType /*attackType*/, bool iscrit) const override
        {
            ...
            if (_spec == BOT_SPEC_MAGE_ARCANE && lvl >= 40)
            {
                if (baseId == ARCANE_MISSILES_DAMAGE_1)
                    fdamage += me->SpellBaseDamageBonusDone(SPELL_SCHOOL_MASK_MAGIC) * 0.45f * me->GetVictim()->CalculateDefaultCoefficient(spellInfo, SPELL_DIRECT_DAMAGE) * me->GetVictim()->CalculateLevelPenalty(spellInfo);
                else if (baseId == ARCANE_BLAST_1)
                    fdamage += me->SpellBaseDamageBonusDone(SPELL_SCHOOL_MASK_MAGIC) * 0.09f * me->GetVictim()->CalculateDefaultCoefficient(spellInfo, SPELL_DIRECT_DAMAGE) * me->GetVictim()->CalculateLevelPenalty(spellInfo);
            }
            ...
            if ((_spec == BOT_SPEC_MAGE_FIRE) &&
                ...
                fdamage += me->SpellBaseDamageBonusDone(SPELL_SCHOOL_MASK_MAGIC) * 0.15f * me->GetVictim()->CalculateDefaultCoefficient(spellInfo, SPELL_DIRECT_DAMAGE) * me->GetVictim()->CalculateLevelPenalty(spellInfo);
        ...
        void SummonBotPet()
        {
            ...
            me->GetNearPoint(myPet, pos.m_positionX, pos.m_positionY, pos.m_positionZ, myPet->GetObjectSize(), 2, me->GetOrientation());
