...
/*
    ...
    {
        return creature->GetBotAI()->GossipHello(player, 0);
    }
...
*/
    struct shaman_botAI : public bot_ai
    {
        ...
        void CheckTotems(uint32 diff)
        {
            ...
            for (uint8 i = 0; i != MAX_TOTEMS; ++i)
            {
                    ...
                    me->GetDistance(_totems[i].second._pos) > _totems[i].second._effradius)
                {
                    ...
                    if (!to)
                    {
                        LOG_ERROR("entities.player", "%s has unexpectingly lost totem in slot %u!", me->GetName().c_str(), i);
        ...
        void ApplyClassDamageMultiplierSpell(int32& damage, SpellNonMeleeDamage& /*damageinfo*/, SpellInfo const* spellInfo, WeaponAttackType /*attackType*/, bool iscrit) const override
        {
            ...
            if ((_spec == BOT_SPEC_SHAMAN_ELEMENTAL) && lvl >= 45)
            {
                if (spellId == GetSpell(CHAIN_LIGHTNING_1) || spellId == GetSpell(LIGHTNING_BOLT_1))
                    flat_mod += me->SpellBaseDamageBonusDone(SPELL_SCHOOL_MASK_MAGIC) * 0.2f * me->GetVictim()->CalculateDefaultCoefficient(spellInfo, SPELL_DIRECT_DAMAGE) * me->GetVictim()->CalculateLevelPenalty(spellInfo);
                else if (spellId == GetSpell(LAVA_BURST_1))
                    flat_mod += me->SpellBaseDamageBonusDone(SPELL_SCHOOL_MASK_MAGIC) * 0.25f * me->GetVictim()->CalculateDefaultCoefficient(spellInfo, SPELL_DIRECT_DAMAGE) * me->GetVictim()->CalculateLevelPenalty(spellInfo);
            }
        ...
        void ApplyClassDamageMultiplierHeal(Unit const* /*victim*/, float& heal, SpellInfo const* spellInfo, DamageEffectType damagetype, uint32 stack) const override
        {
            ...
            if ((_spec == BOT_SPEC_SHAMAN_RESTORATION) && lvl >= 45)
                flat_mod += GetTotalBotStat(BOT_STAT_MOD_INTELLECT) * 0.15f * me->GetVictim()->CalculateDefaultCoefficient(spellInfo, damagetype) * stack * 1.88f * me->GetVictim()->CalculateLevelPenalty(spellInfo) * stack;
            ...
            if ((_spec == BOT_SPEC_SHAMAN_RESTORATION) && lvl >= 55)
            {
                if (spellId == GetSpell(HEALING_WAVE_1))
                    flat_mod += me->SpellBaseHealingBonusDone(SPELL_SCHOOL_MASK_MAGIC) * 0.2f * me->GetVictim()->CalculateDefaultCoefficient(spellInfo, damagetype) * 1.88f * me->GetVictim()->CalculateLevelPenalty(spellInfo) * stack;
                else if (spellId == GetSpell(LESSER_HEALING_WAVE_1))
                    flat_mod += me->SpellBaseHealingBonusDone(SPELL_SCHOOL_MASK_MAGIC) * 0.1f * me->GetVictim()->CalculateDefaultCoefficient(spellInfo, damagetype) * 1.88f * me->GetVictim()->CalculateLevelPenalty(spellInfo) * stack;
            }
        ...
        void DamageTaken(Unit* u, uint32& /*damage*/, DamageEffectType /*damageType*/, SpellSchoolMask /*damageSchoolMask*/) override
        {
            if (!u)
                return;
        ...
        void UnsummonAll() override
        {
            ...
            for (uint8 i = 0; i != MAX_TOTEMS; ++i)
            {
                if (_totems[i].first != ObjectGuid::Empty)
                {
                    ...
                    if (!to)
                    {
                        //LOG_ERROR("entities.player", "%s has no totem in slot %u during remove!", me->GetName().c_str(), i);
                        continue;
                    }
                    to->ToTotem()->UnSummon();
                }
        ...
        void OnBotDespawn(Creature* summon) override
        {
            ...
            switch (totem->m_Properties->Slot)
            {
                ...
                default:
                    LOG_ERROR("entities.player", "OnBotDespawn(): Shaman bot %s has despawned totem %s in unknown slot %u", me->GetName().c_str(), summon->GetName().c_str(), totem->m_Properties->ID);
                    return;
            }
            ...
            //LOG_ERROR("entities.player", "OnBotDespawn(): %s despawned (%s : %u)", summon->GetName().c_str(), summon->IsAlive() ? "alive" : summon->isDying() ? "justdied" : "unk", (uint32)summon->getDeathState());
        ...
        void OnBotSummon(Creature* summon) override
        {
            ...
            switch (totem->m_Properties->Slot)
            {
                ...
                case SUMMON_SLOT_TOTEM_AIR:     slot = T_AIR;   break;
                default:
                    LOG_ERROR("entities.player", "OnBotSummon(): Shaman bot %s has summoned totem %s with unknown type %u", me->GetName().c_str(), summon->GetName().c_str(), totem->m_Properties->ID);
                    return;
            }
            ...
            switch (createSpell)
            {
                ...
                default:
                {
                    LOG_ERROR("scripts", "Unknown totem create spell %u!", createSpell);
            ...
            //LOG_ERROR("entities.player", "shaman bot: summoned %s (type %u) at x=%.2f, y=%.2f, z=%.2f",
  ...
  /*Talent*/lvl >= 40 && isRest ? InitSpellMap(MANA_TIDE_TOTEM_1) : RemoveSpell(MANA_TIDE_TOTEM_1);
        ...
        void InitHeals() override
        {
            ...
            if (InitSpell(me, HEALING_WAVE_1))
            {
                ...
                _heals[HEALING_WAVE_1] = me->SpellHealingBonusDone(me, spellInfo, spellInfo->Effects[0].CalcValue(me), HEAL, spellInfo->GetEffect(EFFECT_0), {});
            }
            else
            ...
            if (InitSpell(me, LESSER_HEALING_WAVE_1))
            {
                ...
                _heals[LESSER_HEALING_WAVE_1] = me->SpellHealingBonusDone(me, spellInfo, spellInfo->Effects[0].CalcValue(me), HEAL, spellInfo->GetEffect(EFFECT_0), {});
            }
            else
            ...
            if (InitSpell(me, RIPTIDE_1))
            {
                ...
                _heals[RIPTIDE_1] = me->SpellHealingBonusDone(me, spellInfo, spellInfo->Effects[0].CalcValue(me), HEAL, spellInfo->GetEffect(EFFECT_0), {});
            }
            else
                _heals[RIPTIDE_1] = 0;
            ...
            if (InitSpell(me, CHAIN_HEAL_1))
            {
                ...
                _heals[CHAIN_HEAL_1] = me->SpellHealingBonusDone(me, spellInfo, spellInfo->Effects[0].CalcValue(me), HEAL, spellInfo->GetEffect(EFFECT_0), {});
            }
            else
    ...
    private:
        ...
        uint32 _getTotemsMask(std::map<uint32 /*type*/, uint32 /*curId*/>& idMap) const
        {
            ...
            for (Unit::AuraApplicationMap::const_iterator itr = aurapps.begin(); itr != aurapps.end(); ++itr)
            {
                ...
                //{
                //    LOG_ERROR("spells","totemMask: unit %s, %s (%u), owner %s (crSp %u, base %u), istotem %u", target->GetName().c_str(),
