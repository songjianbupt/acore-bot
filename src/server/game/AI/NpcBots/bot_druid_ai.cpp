    ...
    struct bot_druid_ai : public bot_ai
    {
        ...
        bool removeShapeshiftForm() override
        {
            ...
            //if (form != FORM_NONE)
            {
                ...
                {
                    //LOG_ERROR("entities.player", "druid_bot::removeShapeshiftForm(): still has poweType %u!", uint32(me->getPowerType()));
                    me->setPowerType(POWER_MANA);
                }
                if (me->GetShapeshiftForm() != FORM_NONE)
                {
                    //LOG_ERROR("entities.player", "druid_bot::removeShapeshiftForm(): still speshifted into %u!", uint32(me->GetShapeshiftForm()));
        ...
        void Attack(uint32 diff)
        {
                ...
                (IsSpellReady(NATURES_SWIFTNESS_1, diff, false) && doCast(me, GetSpell(NATURES_SWIFTNESS_1)))))
            {
                ...
                {
                    Position pos;
                    opponent->GetNearPoint(me, pos.m_positionX, pos.m_positionY, pos.m_positionZ, me->GetObjectSize(), 15.f, opponent->GetAbsoluteAngle(me));
        ...
        void setStats(BotStances form)
        {
            ...
            switch (form)
            {
                ...
                case BOT_STANCE_NONE:
                    ...
                    break;
                default:
                    LOG_ERROR("entities.player", "druid_bot::setStats(): NYI form %u", uint32(form));
                    setStats(BOT_STANCE_NONE);
                    return;
            }
        ...
        void ApplyClassDamageMultiplierSpell(int32& damage, SpellNonMeleeDamage& /*damageinfo*/, SpellInfo const* spellInfo, WeaponAttackType /*attackType*/, bool iscrit) const override
        {
            ...
            if ((_spec == BOT_SPEC_DRUID_BALANCE) && lvl >= 45)
            {
                if (baseId == STARFIRE_1)
                    fdamage += me->SpellBaseDamageBonusDone(SPELL_SCHOOL_MASK_MAGIC) * 0.2f * me->GetVictim()->CalculateDefaultCoefficient(spellInfo, SPELL_DIRECT_DAMAGE) * me->GetVictim()->CalculateLevelPenalty(spellInfo);
                if (baseId == WRATH_1)
                    fdamage += me->SpellBaseDamageBonusDone(SPELL_SCHOOL_MASK_MAGIC) * 0.1f * me->GetVictim()->CalculateDefaultCoefficient(spellInfo, SPELL_DIRECT_DAMAGE) * me->GetVictim()->CalculateLevelPenalty(spellInfo);
            }
        ...
        void ApplyClassDamageMultiplierHeal(Unit const* /*victim*/, float& heal, SpellInfo const* spellInfo, DamageEffectType damagetype, uint32 stack) const override
        {
            ...
            if ((_spec == BOT_SPEC_DRUID_RESTORATION) && lvl >= 35)
            {
                if (baseId == HEALING_TOUCH_1)
                    flat_mod += me->SpellBaseHealingBonusDone(SPELL_SCHOOL_MASK_MAGIC) * 0.4f * me->GetVictim()->CalculateDefaultCoefficient(spellInfo, damagetype) * 1.88f * me->GetVictim()->CalculateLevelPenalty(spellInfo) * stack;
        ...
        void SpellHitTarget(WorldObject* wtarget, SpellInfo const* spell) override
        {
            ...
            if (_form == DRUID_CAT_FORM)
            {
                    ...
                    baseId == RAKE_1 || baseId == RAVAGE_1 || baseId == SHRED_1)
                {
                    comboPoints++;
                    //debug
                    //LOG_ERROR("entities.player", "druid_bot CP GEN: %s adds 1, now %u", spell->SpellName[0], uint32(comboPoints));
                    if (primalFuryProc)
                    {
                        comboPoints++;
                        //debug
                        //LOG_ERROR("entities.player", "druid_bot CP EX: now %u", uint32(comboPoints));
                    }
                    if (comboPoints > 5)
                    {
                        comboPoints = 5;
                        //debug
                        //LOG_ERROR("entities.player", "druid_bot CP NOR: now %u", uint32(comboPoints));
                    }
                }
                ...
                {
                    //debug
                    //LOG_ERROR("entities.player", "druid_bot CP SPEND1: %u to 0", uint32(comboPoints));
     ...
     /*tal*/lvl >= 60 && isRest ? InitSpellMap(WILD_GROWTH_1) : RemoveSpell(WILD_GROWTH_1);
        ...
        void InitHeals() override
        {
            ...
            if (InitSpell(me, NOURISH_1))
            {
                ...
                _heals[NOURISH_1] = me->SpellHealingBonusDone(me, spellInfo, spellInfo->Effects[0].CalcValue(me), HEAL, spellInfo->GetEffect(EFFECT_0), {});
            }
            else
                _heals[NOURISH_1] = 0;
            ...
            if (InitSpell(me, REGROWTH_1))
            {
                ...
                _heals[REGROWTH_1] = me->SpellHealingBonusDone(me, spellInfo, spellInfo->Effects[0].CalcValue(me), HEAL, spellInfo->GetEffect(EFFECT_0), {});
            }
            else
                _heals[REGROWTH_1] = 0;
            ...
            if (InitSpell(me, HEALING_TOUCH_1))
            {
                ...
                _heals[HEALING_TOUCH_1] = me->SpellHealingBonusDone(me, spellInfo, spellInfo->Effects[0].CalcValue(me), HEAL, spellInfo->GetEffect(EFFECT_0), {});
            }
            else
            ...
            if (InitSpell(me, REJUVENATION_1))
            {
                ...
                _heals[REJUVENATION_1] = me->SpellHealingBonusDone(me, spellInfo, spellInfo->Effects[0].CalcValue(me), DOT, spellInfo->GetEffect(EFFECT_0), {});
            }
            else
