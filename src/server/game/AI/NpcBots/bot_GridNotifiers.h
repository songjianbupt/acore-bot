    ...
    public:
        ...
        bool operator()(Unit const* u) const
        {
            ...
            for (Unit::AuraApplicationMap::const_iterator itr = Auras.begin(); itr != Auras.end(); ++itr)
            {
                ...
                if (itr->first == spell)
                    if (caster == ObjectGuid(uint64(0)) || auraApp->GetBase()->GetCasterGUID() == caster)
                        return true;
            }
            return false;
        }
...
class PolyUnitCheck
{
    public:
        ...
        bool operator()(Unit * u) const
        {
            ...
            if (u->IsPolymorphed() ||
                u->isFrozen() ||
                u->ToCreature()->GetMovementTemplate().IsRooted() ||
...
class RootUnitCheck
{
    public:
        ...
        bool operator()(Unit * u) const
        {
            ...
            if (!u->isTargetableForAttack(false))
                return false;
            if (u->isFrozen() || u->ToCreature()->GetMovementTemplate().IsRooted())
                return false;
            if (!u->getAttackers().empty())
                return false;
            if (u->GetVictim() && !m_ai->IsInBotParty(u->GetVictim()))
