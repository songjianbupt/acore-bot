...
*/
    struct priest_botAI : public bot_ai
    {
        ...
        bool HealTarget(Unit* target, uint32 diff) override
        {
            ...
            //LOG_ERROR("entities.player", "priest_bot:HealTarget(): %s's pct %u, hppctps %i, epct %i",
        ...
        void ApplyClassDamageMultiplierSpell(int32& damage, SpellNonMeleeDamage& damageinfo, SpellInfo const* spellInfo, WeaponAttackType /*attackType*/, bool iscrit) const override
        {
            ...
            if ((_spec == BOT_SPEC_PRIEST_SHADOW) && lvl >= 45)
            {
                if (baseId == MIND_BLAST_1 || baseId == MIND_FLAY_DAMAGE || baseId == MIND_SEAR_DAMAGE_1)
                    fdamage += me->SpellBaseDamageBonusDone(SPELL_SCHOOL_MASK_MAGIC) * 0.15f * me->GetVictim()->CalculateDefaultCoefficient(spellInfo, DIRECT_DAMAGE) * me->GetVictim()->CalculateLevelPenalty(spellInfo);
            }
        ...
        void ApplyClassDamageMultiplierHeal(Unit const* victim, float& heal, SpellInfo const* spellInfo, DamageEffectType damagetype, uint32 stack) const override
        {
            ...
            if ((_spec == BOT_SPEC_PRIEST_HOLY) && lvl >= 50 && baseId == RENEW_1)
                flat_mod += me->SpellBaseHealingBonusDone(SPELL_SCHOOL_MASK_MAGIC) * 0.15f * me->GetVictim()->CalculateDefaultCoefficient(spellInfo, damagetype) * 1.88f * me->GetVictim()->CalculateLevelPenalty(spellInfo) * stack;
  ...
  /*Talent*/lvl >= 60 && isShad ? InitSpellMap(DISPERSION_1) : RemoveSpell(DISPERSION_1);
        ...
        void InitHeals() override
        {
            ...
            if (InitSpell(me, HEAL))
            {
                ...
                _heals[HEAL] = me->SpellHealingBonusDone(me, spellInfo, spellInfo->Effects[0].CalcValue(me), DamageEffectType(3), spellInfo->GetEffect(EFFECT_0), {});
            }
            else
                _heals[HEAL] = 0;
            ...
            if (InitSpell(me, FLASH_HEAL_1))
            {
                ...
                _heals[FLASH_HEAL_1] = me->SpellHealingBonusDone(me, spellInfo, spellInfo->Effects[0].CalcValue(me), DamageEffectType(3), spellInfo->GetEffect(EFFECT_0), {});
            }
            else
            ...
            if (InitSpell(me, PENANCE_HEAL_1))
            {
                ...
                _heals[PENANCE_1] = me->SpellHealingBonusDone(me, spellInfo, spellInfo->Effects[0].CalcValue(me), DamageEffectType(3), spellInfo->GetEffect(EFFECT_0), {});
            }
            else
                _heals[PENANCE_1] = 0;
            ...
            if (InitSpell(me, DESPERATE_PRAYER_1))
            {
                ...
                _heals[DESPERATE_PRAYER_1] = me->SpellHealingBonusDone(me, spellInfo, spellInfo->Effects[0].CalcValue(me), DamageEffectType(3), spellInfo->GetEffect(EFFECT_0), {});
            }
            else
