...
*/
    struct rogue_botAI : public bot_ai
    {
        ...
        void SpellHitTarget(WorldObject* wtarget, SpellInfo const* spell) override
        {
            ...
            {
                ++comboPoints;
                //LOG_ERROR("entities.player", "rogue_bot CP GEN2: %s adds 1, now %u", spell->SpellName[0], uint32(comboPoints));
                if (comboPoints > 5)
                {
                    comboPoints = 5;
                    //LOG_ERROR("entities.player", "rogue_bot CP NOR2: now %u", uint32(comboPoints));
                }
            }
                ...
                baseId == AMBUSH_1 || baseId == GARROTE_1 || baseId == CHEAP_SHOT_1/* || baseId == SHIV_1*/)
            {
                ...
                //LOG_ERROR("entities.player", "rogue_bot CP GEN1: %s adds %u, now %u",
                ...
                {
                    ++comboPoints;
                    //LOG_ERROR("entities.player", "rogue_bot CP GEN1: glyphSS proc, now %u", uint32(comboPoints));
                }

                if (comboPoints > 5)
                {
                    comboPoints = 5;
                    //LOG_ERROR("entities.player", "rogue_bot CP NOR1: now %u", uint32(comboPoints));
                }
            }
            ...
            else if (spell->NeedsComboPoints() && comboPoints)
            {
                ...
                //LOG_ERROR("entities.player", "rogue_bot CP SPEND1: %u to 0", tempCP);
