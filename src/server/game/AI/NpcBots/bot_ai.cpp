...
const std::string& bot_ai::LocalizedNpcText(Player const* forPlayer, uint32 textId)
{
    ...
    {
        LOG_ERROR("entities.player", "NPCBots: bot text string #%u is not localized, at least for %s",
...
bool bot_ai::SetBotOwner(Player* newowner)
{
    ...
    {
        LOG_ERROR("entities.player", "bot_ai::SetBotOwner(): bot %s (id: %u) has master %s while trying to set to %s...",
            me->GetName().c_str(), me->GetEntry(), master->GetName().c_str(), newowner->GetName().c_str());
        return false;
    }
    if (!IAmFree())
    {
        LOG_ERROR("entities.player", "bot_ai::SetBotOwner(): minion bot %s (id: %u) IS NOT FREE (has master %s) while trying to set to %s",
            me->GetName().c_str(), me->GetEntry(), master->GetName().c_str(), newowner->GetName().c_str());
        return false;
    }
    ...
    {
        //LOG_ERROR("entities.player", "bot_ai::SetBotOwner(): player %s (%s) can't add bot %s (FATAL), removing...",
        ...
        //    stmt->SetData<uint32>(0, uint32(0));
        //    stmt->SetData<uint32>(1, me->GetEntry());
        //    CharacterDatabase.Execute(stmt);
        //}

        if (_ownerGuid)
        {
            LOG_ERROR("entities.player", "bot_ai::SetBotOwner(): %s's master %s (%s) is found but bot failed to set owner (fatal)! Unbinding bot temporarily (until server restart)...",
...
void bot_ai::CheckOwnerExpiry()
{
    ...
    QueryResult result = accId ? LoginDatabase.Query("SELECT UNIX_TIMESTAMP(last_login) FROM account WHERE id = %u", accId) : nullptr;
    ...
    if (timeNow >= lastLoginTime + expireTime)
    {
        ...
        LOG_DEBUG("server.loading", ">> %s's (guid: %u) ownership over bot %s (%u) has expired!",
        ...
        for (uint8 i = 0; i != BOT_INVENTORY_SIZE; ++i)
            stmt->SetData<uint32>(i, npcBotData->equips[i]);
...
SpellCastResult bot_ai::CheckBotCast(Unit const* victim, uint32 spellId) const
{
        ...
        (victim->getLevel() + 10) < int32(spellInfo->GetFirstRankSpell()->SpellLevel))
    {
        for (uint8 i = 0; i < MAX_SPELL_EFFECTS; ++i)
        {
            ...
            {
                //LOG_ERROR("scripts", "CheckBotCast(): deny cast of %s by %s on low-level target %s (lvl %u)",
    ...
    switch (_botclass)
    {
        ...
        case BOT_CLASS_NECROMANCER:
            break;
        default:
            LOG_ERROR("entities.player", "CheckBotCast(): Unknown bot class %u", _botclass);
            break;
    }
...
bool bot_ai::doCast(Unit* victim, uint32 spellId, TriggerCastFlags flags)
{
    ...
    {
        LOG_ERROR("entities.player", "bot_ai::doCast(): %s (bot class %u) tried to cast spell %u on a dead target %s",
    ...
    if (me->isMoving() && !HasBotCommandState(BOT_COMMAND_ISSUED_ORDER) &&
        ...
        //Mind Flay (Rank 8)
        || spellId == 48155) &&
        !(m_botSpellInfo->Attributes & SPELL_ATTR0_ON_NEXT_SWING_NO_DAMAGE) && !m_botSpellInfo->IsAutoRepeatRangedSpell() &&
        !(flags & TRIGGERED_CAST_DIRECTLY) && (m_botSpellInfo->IsChanneled() || m_botSpellInfo->CalcCastTime()))
    {
        ...
        //    LOG_ERROR("entities.player", "bot_ai::doCast(): spell %u interrupts movement", spellId);
        me->BotStopMovement();
    }
    ...
    //debug
    //LOG_ERROR("entities.player", "CheckBotCast(): %s (%u) by %s on %s", m_botSpellInfo->SpellName[0], spellId, me->GetName().c_str(), victim->GetName().c_str());
...
/*
    ...
    if (result != SPELL_CAST_OK)
        LOG_ERROR("entities.player", "doCast(): %s (%u) by %s on %s failed with %u",
...
*/
    ...
    if (!casted)
    {
        ...
        {
            if (DEBUG_BOT_ORDERS)
                LOG_ERROR("entities.player", "doCast(): ordered spell %u is not casted!", m_botSpellInfo->Id);
...
void bot_ai::_calculatePos(Position& pos) const
{
    ...
    if (bmover->CanFly())
    {
        ...
        for (uint8 i = 0; i != 5 + over; ++i)
        {
            ...
            mmover->GetNearPoint(bmover, x, y, z, bmover->GetObjectSize(), mydist, angle);
...
void bot_ai::BotMovement(BotMovementType type, Position const* pos, Unit* target, bool generatePath) const
{
    ...
    switch (type)
    {
        ...
        case BOT_MOVE_POINT:
            ...
            break;
        default:
            LOG_ERROR("scripts", "BotMovement: unhandled bot movement type %u", uint32(type));
            return;
    }
}
...
void bot_ai::RezGroup(uint32 REZZ)
{
    ...
    //LOG_ERROR("entities.unit", "RezGroup: %s found %u targets", me->GetName().c_str(), uint32(targets.size()));
...
bool bot_ai::CanAffectVictim(uint32 schoolMask) const
{
    ...
    {
        LOG_ERROR("entities.player", "bot_ai::CanDamageVictim(): schoolMask is not present (class = %u)", _botclass);
        return false;
    }
...
void bot_ai::SetStats(bool force)
{
    ...
    /*LOG_ERROR("entities.player", "*etStats(): Updating bot %s, class: %u, race: %u, level %u, master: %s",
    ...
    switch (myclass)
    {
        ...
        default:
            LOG_ERROR("entities.player", "minion_ai: *etStats():Init - unknown bot class %u, real class: %u, _botclass: %u", myclass, GetPlayerClass(), _botclass);
            break;
    }
    ...
    switch (myclass)
    {
        ...
        case BOT_CLASS_NECROMANCER:
            strmult = 0.f; agimult = 0.f; break;
        default:
            LOG_ERROR("entities.player", "_MeleeDamageUpdate(): NIY myclass %u!", uint32(myclass));
            strmult = 0.f; agimult = 0.f; break;
    }
    ...
    atpower *= ap_mod;
    me->HandleStatModifier(UNIT_MOD_ATTACK_POWER, BASE_VALUE, atpower, true);
        ...
        _botclass == BOT_CLASS_DARK_RANGER)
    {
        ...
        me->HandleStatModifier(UNIT_MOD_ATTACK_POWER_RANGED, BASE_VALUE, atpower, true);
    ...
    value *= armor_mod;
    me->HandleStatModifier(UNIT_MOD_ARMOR, BASE_VALUE, value, true);
...
void bot_ai::RefreshAura(uint32 spellId, int8 count, Unit* target) const
{
    if (count < 0 || count > 1)
    {
        LOG_ERROR("entities.player", "bot_ai::RefreshAura(): count is out of bounds (%i) for bot %s (botclass: %u, entry: %u)",
    ...
    if (!spellId)
    {
        LOG_ERROR("entities.player", "bot_ai::RefreshAura(): spellId is 0 for bot %s (botclass: %u, entry: %u)",
    ...
    if (!spellInfo)
    {
        LOG_ERROR("entities.player", "bot_ai::RefreshAura(): Invalid spellInfo for spell %u! Bot - %s (botclass: %u, entry: %u)",
...
*/
    ...
    if (!IAmFree() && (HasRole(BOT_ROLE_RANGED) || HasVehicleRoleOverride(BOT_ROLE_RANGED)))
    {
        ...
        //LOG_ERROR("entities.player", "bot %s ranged foldist %.2f spelldist %.2f", me->GetName().c_str(), foldist, spelldist);
    }
    ...
    if (dropTarget)
    {
        //LOG_ERROR("entities.player", "bot %s cannot attack target %s, too far away or not in LoS", me->GetName().c_str(), mytar ? mytar->GetName().c_str() : "unk");
        return nullptr;
    }
...
bool bot_ai::CheckAttackTarget()
{
    ...
    switch (_botclass)
    {
        case BOT_CLASS_DRUID:
            switch (GetBotStance())
            {
                ...
                case BOT_STANCE_NONE:
                    ...
                    break;
                default:
                    LOG_ERROR("entities.player", "bot_ai::CheckAttackTarget(): druid has NYI bot stance %u", uint32(GetBotStance()));
                    break;
            }
            break;
        case BOT_CLASS_PRIEST:
        ...
        case BOT_CLASS_DARK_RANGER:
            break;
        default:
            LOG_ERROR("entities.player", "bot_ai: CheckAttackTarget() - unknown bot class %u", _botclass);
            return false;
    }
    ...
    if (!opponent)
    {
        ...
        if (me->GetVictim() || me->IsInCombat()/* || !me->getThreatMgr().isThreatListEmpty()*/)
        {
        ...
        if (Group const* gr = master->GetGroup())
        {
                ...
                Acore::Containers::SelectRandomContainerElement(spines))
            {
                if (GameObject const* spine = u->GetFirstGameObjectById(185584)) // Naj'entus Spine
                {
                    ...
                    if (spine->AI() && spine->AI()->GossipHello(receiver, 0))
                    {
...
void bot_ai::CalculateAoeSpots(Unit const* unit, AoeSpotsVec& spots)
{
    ...
    //if (!doList.empty())
    //    LOG_ERROR("scripts", "CalculateAoeSpots %u aoes around %s", uint32(doList.size()), unit->GetName().c_str());
    ...
    for (std::list<WorldObject*>::const_iterator ci = doList.begin(); ci != doList.end(); ++ci)
    {
        ...
        {
            //LOG_ERROR("scripts", "CalculateAoeSpots found %s's aoe %s (%u) radius %.2f size %.2f",
...
void bot_ai::CalculateAttackPos(Unit* target, Position& pos, bool& force) const
{
    ...
    if (me->GetVehicle())
    {
        ...
        for (uint8 i = 0; i < 4; ++i)
        {
            if (me->GetVehicleBase()->CanFly())
            {
                ...
                target->GetNearPoint(me->GetVehicleBase(), tx, ty, tz, target->GetObjectSize(), dist, Position::NormalizeOrientation(angle));
    ...
    if (!safespots.empty())
    {
        ...
        //LOG_ERROR("scripts", "CalculateAttackPos %u safe spots, chosen at dist %.2f", uint32(safespots.size()), mindist);
...
//        return;
    ...
    //LOG_ERROR("entities.player", "AdjustTankingPosition(): atts %u, behind %u", uint32(myattackers.size()), bCount);
...
void bot_ai::MoveBehind(Unit const* target) const
{
    ...
    if ((_botclass == BOT_CLASS_ROGUE || GetBotStance() == DRUID_CAT_FORM) ? (!targetMe || cced || isPlayer) : (!targetMe && (!cced || isPlayer)))
    {
        ...
        Position position;
        target->GetNearPoint(me, position.m_positionX, position.m_positionY, position.m_positionZ, me->GetObjectSize(), mydist, myangle);
...
Unit* bot_ai::FindAffectedTarget(uint32 spellId, ObjectGuid caster, float dist, uint8 hostile) const
{
    ...
    {
        LOG_ERROR("entities.player", "bot_ai::FindAffectedTarget(): hostile = %u Setting to ALL...", hostile);
        hostile = 255;
    }
...
uint32 bot_ai::InitSpell(Unit const* caster, uint32 spell)
{
    ...
    if (!info)
    {
        LOG_ERROR("entities.player", "InitSpell(): No SpellInfo found for spell %u", spell);
...
void bot_ai::InitSpellMap(uint32 basespell, bool forceadd, bool forwardRank)
{
    ...
    if (!info)
    {
        LOG_ERROR("entities.player", "bot_ai::InitSpellMap(): No SpellInfo found for base spell %u", basespell);
    ...
    if (npcBotData->disabled_spells.find(basespell) != npcBotData->disabled_spells.end())
    {
        ...
        //LOG_ERROR("entities.player", "bot_ai::InitSpellMap(): %s (%u -> %u) is disabled for %s!",
...
void bot_ai::SetSpellCategoryCooldown(SpellInfo const* spellInfo, uint32 msCooldown)
{
    ...
    for (BotSpellMap::const_iterator itr = _spells.begin(); itr != _spells.end(); ++itr)
    {
        ...
        if (info && itr->first == spellInfo->Id && info->GetCategory() != category)
        {
            ...
            {
                LOG_ERROR("scripts", "Warning: SetSpellCategoryCooldown: %u has baseId %u but category %u, not %u!",
...
void bot_ai::ReleaseSpellCooldown(uint32 basespell)
{
    ...
    {
        LOG_ERROR("spells", "bot_ai::ReleaseSpellCooldown is called for wrong spell %u!", basespell);
        return;
    }
    ...
    switch (myrace)
    {
        ...
        case RACE_DRAENEI:
            switch (myclass)
            {
                ...
                default:
                    LOG_ERROR("entities.player", "RaceSpellForClass(): unknows race:class combo %u, %u", uint32(myrace), uint32(myclass));
                    return 0;
            }
            break;
        default:
...
void bot_ai::_OnHealthUpdate() const
{
    ...
    //LOG_ERROR("entities.player", "class base health: %u", m_basehp);
    ...
    //LOG_ERROR("entities.player", "total base health: %u", m_totalhp);
    ...
    me->HandleStatModifier(UNIT_MOD_HEALTH, BASE_VALUE, float(m_totalhp), true); //replaces base hp at max lvl
    me->UpdateMaxHealth(); //will use our values we just set (update base health and buffs)
    //LOG_ERROR("entities.player", "overall hp: %u", me->GetMaxHealth());
...
void bot_ai::_OnManaUpdate() const
{
    ...
    me->HandleStatModifier(UNIT_MOD_MANA, BASE_VALUE, m_basemana, true);
...
void bot_ai::_UpdateWMOArea()
{
    ...
    if (WMOAreaTableEntry const* wmoEntry = GetWMOAreaTableEntryByTripple(rootId, adtId, groupId))
    {
        ...
        //LOG_ERROR("scripts", "_UpdateWMOArea(): bot %s: area %u, wmoarea %u", me->GetName().c_str(), _lastAreaId, _lastWMOAreaId);
    }
}
...
void bot_ai::OnSpellHitTarget(Unit* /*target*/, SpellInfo const* spell)
{
    if (me->GetVehicle())
    {
        ...
        if (spellId == 56091 || spellId == 57090)
        {
            ...
            //LOG_ERROR("scripts", "OnBotSpellGo(): veh cp spell %u now cp %u", curInfo->Id, uint32(vehcomboPoints));
        }
        //Engulf in Flames, Life Burst, Flame Shield   moved to globalupdate
        if (spellId == 56092 || spellId == 57143 || spellId == 57108)
        {
            vehcomboPoints = 0;
            //LOG_ERROR("scripts", "OnSpellHitTarget(): veh cp waster %u", curInfo->Id);
        }
    }
}
//Update delay
...
bool bot_ai::OnGossipSelect(Player* player, Creature* creature/* == me*/, uint32 sender, uint32 action)
{
    ...
    switch (sender)
    {
        ...
        case GOSSIP_SENDER_EQUIPMENT_SHOW: //equips change s2: send list of equippable items
        {
            ...
            //LOG_ERROR("entities.player", "OnGossipSelect(bot): added %u item(s) to list of %s (requester: %s)",
        ...
        case GOSSIP_SENDER_ABILITIES_USAGE_LIST_SUPPORT:
        {
            ...
            for (std::vector<uint32>::const_iterator itr = myspells->begin(); itr != myspells->end(); ++itr)
            {
                ...
                {
                    LOG_ERROR("scripts", "bot_ai: gossip abilities list overflow with sender %u for bot class %u!",
        ...
        case GOSSIP_SENDER_SPEC:
        {
            ...
            switch (_botclass)
            {
                ...
                default:
                    LOG_ERROR("entities.unit", "bot_ai:GOSSIP_SENDER_SPEC called for class %u with no specs!", uint32(_botclass));
                    return true;
            }
...
void bot_ai::OnOwnerDamagedBy(Unit* attacker)
{
    ...
    switch (_botclass)
    {
        ...
        case BOT_CLASS_ARCHMAGE:
            ...
            break;
        default:
            //LOG_ERROR("entities.player", "minion_ai: OnOwnerDamagedBy() - unknown bot class %u", uint8(_botclass));
            break;
    }
...
bool bot_ai::_canLootItemForPlayer(Player* player, Creature* creature, uint8 slot) const
{
    ...
    //{
    //    //LOG_ERROR("scripts", "can't loot item %u, no item", slot);
    //    return false;
    //}

    //if (!qitem && item->is_blocked)
    //{
    //    //LOG_ERROR("scripts", "can't loot item %u (%u), blocked", slot, item->itemid);
    //    return false;
    //}

    //if (!item->rollWinnerGUID.IsEmpty() && item->rollWinnerGUID != player->GetGUID())
    //{
    //    //LOG_ERROR("scripts", "can't loot item %u (%u), roll won", slot, item->itemid);
    //    return false;
    //}
    ...
    ////LOG_ERROR("scripts", "can't loot item %u (%u), can't store", slot, item->itemid);
    //return false;
}
bool bot_ai::_canLootCreatureForPlayer(Player* player, Creature* creature, uint32 lootQualityMask, uint32 lootThreshold) const
{
    ...
    for (std::vector<LootItem>::const_iterator i = creature->loot.items.begin(); i != creature->loot.items.end(); ++i)
    {
        ...
        {
            //LOG_ERROR("scripts", "item %u is blocked", i->itemid);
            continue;
        }

        if (!i->rollWinnerGUID.IsEmpty() && i->rollWinnerGUID != player->GetGUID())
        {
            //LOG_ERROR("scripts", "can't loot item %u (%u), roll won", slot, i->itemid);
            continue;
        }
        ...
        if (!itemProto)
        {
            //LOG_ERROR("scripts", "no item proto for itemId %u", i->itemid);
            return false;
        }

        if (itemProto->Quality >= lootThreshold)
        {
            //LOG_ERROR("scripts", "item %u group quality threshold mismatch", i->itemid);
            continue;
        }

        if (!((1 << itemProto->Quality) & lootQualityMask))
        {
            //LOG_ERROR("scripts", "item %u lootQualityMask mismatch", i->itemid);
            continue;
        }
...
bool bot_ai::_canLootCreature(Creature* creature) const
{
    ...
    //LOG_ERROR("scripts", "lootQualityMask %u, lootThreshold %u", lootQualityMask, lootThreshold);
    ...
    for (std::vector<LootItem>::const_iterator ci = loot->quest_items.begin(); ci != loot->quest_items.end(); ++ci)
    {
        ...
        if (!itemProto)
        {
            //LOG_ERROR("scripts", "no item proto for itemId %u", ci->itemid);
            return false;
        }
...
void bot_ai::_autoLootCreatureItems(Player* receiver, Creature* creature, uint32 lootQualityMask, uint32 lootThreshold) const
{
    ...
    for (std::vector<LootItem>::iterator i = creature->loot.items.begin(); i != creature->loot.items.end(); ++i)
    {
        ...
        {
            //LOG_ERROR("scripts", "looting %s (%u), quality %u, threshold %u",
...
bool bot_ai::_resetEquipment(uint8 slot, ObjectGuid::LowType receiver)
{
    ...
    {
        LOG_ERROR("entities.player", "minion_ai::_resetEquipment(): player %s (%s) failed to reset equipment for bot %s (id: %u) in slot %u",
        ...
        switch (enchant_display_type)
        {
            ...
            case ITEM_ENCHANTMENT_TYPE_EQUIP_SPELL:
                if (enchant_spell_id)
                {
                    ...
                    me->CastCustomSpell(enchant_spell_id, SPELLVALUE_BASE_POINT0, basepoints);
                }
                break;
...
void bot_ai::ApplyItemEquipSpells(Item* item, bool apply)
{
    ...
    for (uint8 i = 0; i != MAX_ITEM_PROTO_SPELLS; ++i)
    {
        ...
        if (apply)
            ...
            me->CastSpell(me, spellInfo->Id, true, item);
        else
...
void bot_ai::ApplyItemSetBonuses(Item* item, bool apply)
{
    if (item) //(un)equip, NOT from loops
    {
        ...
        for (uint8 i = 0; i != MAX_ITEM_SET_SPELLS; ++i)
        {
            ...
            //LOG_ERROR("entities.player", "ApplyItemSetBonusesB: %s's %s, %s (%u), %s (%u), icount %u",
    ...
    for (std::list<uint32>::const_iterator itr = itemSets.begin(); itr != itemSets.end(); ++itr)
    {
        ...
        for (uint8 j = 0; j != MAX_ITEM_SET_SPELLS; ++j)
        {
            ...
            //LOG_ERROR("entities.player", "ApplyItemSetBonusesB (all): %s's %s, %s (%u), %s (%u), c %u, req %u",
...
float bot_ai::_getItemGearScore(Item const* item, uint8 forslot) const
{
    ...
    //LOG_ERROR("scripts", "_getItemGearScore for %u - %s", item->GetEntry(), proto->Name1.c_str());

    ItemStatBonus istats = {};
    //for (uint8 i = 0; i != MAX_BOT_ITEM_MOD; ++i)
    //    LOG_ERROR("scripts", "_getItemGearScore at %u %i", uint32(i), istats[i]);
    ...
    //for (uint8 i = 0; i != MAX_BOT_ITEM_MOD; ++i)
    //    LOG_ERROR("scripts", "_getItemGearScore total %u %i", uint32(i), istats[i]);
...
{
    return target ? target->HasUnitState(UNIT_STATE_CONFUSED | UNIT_STATE_STUNNED | UNIT_STATE_FLEEING | UNIT_STATE_DISTRACTED | UNIT_STATE_CONFUSED_MOVE | UNIT_STATE_FLEEING_MOVE) || (root && (target->HasUnitState(UNIT_STATE_ROOT) || target->isFrozen() || target->ToCreature()->GetMovementTemplate().IsRooted())) : true;
}
...
void bot_ai::ApplyRacials()
{
    ...
    switch (myrace)
    {
        ...
        case RACE_DRAENEI:
            ...
            break;
        default:
            //LOG_ERROR("entities.player", "bot_ai::ApplyRacePassives(): unknown race %u for bot %s (%u)", uint32(me->getRace()), me->GetName().c_str(), me->GetEntry());
            return;
    }
}
...
void bot_ai::InitSpec()
{
    ...
    //LOG_ERROR("entities.unit", "bot_ai::InitSpec(): bot %u class %u spec: %u", me->GetEntry(), uint32(_botclass), uint32(spec));

    if (spec < BOT_SPEC_BEGIN || spec > BOT_SPEC_END)
    {
        LOG_ERROR("entities.unit", "bot_ai::InitSpec(): spec (%u) is out of range for bot %u! Falling to default (1)...",
...
void bot_ai::InitEquips()
{
    ...
    else
    {
        do
        {
            ...
            ASSERT(found);
            //ItemTemplate const* proto = item->GetTemplate();
            //LOG_ERROR("entities.player", "minion_ai::InitEquips(): bot %s (id: %u): found item: for slot %u: %s (id: %u, guidLow: %u)",
...
void bot_ai::_LocalizeItem(Player const* forPlayer, std::string &itemName, std::string &suffix, Item const* item) const
{
    ...
    if (randomPropId > 0)
    {
        if (ItemRandomPropertiesEntry const* item_rand = sItemRandomPropertiesStore.LookupEntry(randomPropId))
        {
            ...
            //if (suffs)
            {
                ...
                //    LOG_ERROR("entities.player", "bot_ai::_LocalizeItem(): rand prop suffix for loc %u = %s", i, suffs[i]);
                suffix = suffs[loc];
            }
        }
    }
    else
    {
        if (ItemRandomSuffixEntry const* item_rand = sItemRandomSuffixStore.LookupEntry(-randomPropId))
        {
            ...
            {
                //for (uint8 i = 0; i != MAX_LOCALES; ++i)
                //    LOG_ERROR("entities.player", "bot_ai::_LocalizeItem(): rand suff suffix for loc %u = %s", i, suffs[i]);
                suffix = suffs[loc];
            }
        }
    }
...
void bot_ai::JustDied(Unit*)
{
    ...
    if (IsTempBot())
    {
        //LOG_ERROR("entities.player", "Unsummoning temp bot %s (%s), owner: %s (%s)...",
...
void bot_ai::OnBotSpellGo(Spell const* spell, bool ok)
{
    ...
    {
        if (DEBUG_BOT_ORDERS)
            LOG_ERROR("entities.player", "doCast(): ordered spell %u by %s was %s!",
...
void bot_ai::OnBotOwnerSpellGo(Spell const* spell, bool ok)
{
    ...
    //LOG_ERROR("entities.player", "OnBotOwnerSpellGo(): %u by %s", spellInfo->Id, master->GetName().c_str());
...
void bot_ai::OnBotSpellInterrupted(SpellSchoolMask schoolMask, uint32 unTimeMs)
{
    ...
    for (BotSpellMap::const_iterator itr = _spells.begin(); itr != _spells.end(); ++itr)
    {
        ...
        {
            if (DEBUG_BOT_ORDERS)
                LOG_ERROR("entities.player", "doCast(): ordered spell %u was interrupted!", info->Id);
            CompleteOrder(_orders.front());
        }

        itr->second->cooldown += unTimeMs;
        //LOG_ERROR("entities.player", "OnBotSpellInterrupted(): Adding cooldown (%u, new: %u) to spell %s (id: %u, schoolmask: %u), reqSchoolMask = %u",
...
void bot_ai::CastBotItemCombatSpell(Unit* target, WeaponAttackType attType, uint32 procVictim, uint32 procEx)
{
    ...
    if (!me->CanUseAttackType(attType) || me->GetShapeshiftForm() == FORM_GHOSTWOLF)
        return;
...
{
    //if (!sScriptMgr->CanCastItemCombatSpell(me, target, attType, procVictim, procEx, item, proto))
    //    return;
        ...
        //if (damageInfo->procVictim & PROC_FLAG_TAKEN_ANY_DAMAGE)
    {
        for (uint8 i = 0; i < MAX_ITEM_SPELLS; ++i)
        {
            ...
            //if (roll_chance_f(chance) && sScriptMgr->OnCastItemCombatSpell(me, target, spellInfo, item))
            if (roll_chance_f(chance))
                me->CastSpell(target, spellInfo->Id, TriggerCastFlags(TRIGGERED_FULL_MASK & ~TRIGGERED_IGNORE_SPELL_AND_CATEGORY_CD), item);
        }
    }
        ...
        for (uint8 s = 0; s < MAX_SPELL_ITEM_ENCHANTMENT_EFFECTS; ++s)
        {
            ...
            if (!spellInfo)
            {
                LOG_ERROR("entities.player", "Player::CastItemCombatSpell({}, name: {}, enchant: {}): unknown spell {} is casted, ignoring...",
                               GetGUID().ToString(), me->GetName(), pEnchant->ID, pEnchant->spellid[s]);
                continue;
            }
            ...
            // Apply spell mods
            //me->ApplySpellMod(pEnchant->spellid[s], SPELLMOD_CHANCE_OF_SUCCESS, chance);
            ...
            if (roll_chance_f(chance))
            {
                ...
                //    if (!--charges)
                //    {
                //        me->ApplyEnchantment(item, EnchantmentSlot(e_slot), false);
                ...
                if (spellInfo->IsPositive())
                    me->CastSpell(me, spellInfo, TriggerCastFlags(TRIGGERED_FULL_MASK & ~TRIGGERED_IGNORE_SPELL_AND_CATEGORY_CD), item);
                else
                    me->CastSpell(target, spellInfo, TriggerCastFlags(TRIGGERED_FULL_MASK & ~TRIGGERED_IGNORE_SPELL_AND_CATEGORY_CD), item);
            }
        }
    }
}

//void bot_ai::CastBotItemCombatSpell(DamageInfo const& damageInfo, CalcDamageInfo const& caldamageInfo)
//{
...
//        }
//
//        CastBotItemCombatSpell(damageInfo, caldamageInfo, item, proto);
//    }
//}
//
//void bot_ai::CastBotItemCombatSpell(DamageInfo const& damageInfo, CalcDamageInfo const& caldamageInfo, Item* item, ItemTemplate const* proto)
//{
...
//    bool canTrigger = caldamageInfo.procVictim & PROC_FLAG_TAKEN_DAMAGE;
//    if (canTrigger)
//    {
//        for (uint8 i = 0; i != MAX_ITEM_PROTO_SPELLS; ++i)
//        {
...
//            {
//                // Check hit/crit/dodge/parry requirement
//                if ((entry->procEx & caldamageInfo.procEx) == 0)
//                    continue;
//            }
//            else
//            {
...
//            {
//                //CastSpellExtraArgs args(item);
//                me->CastSpell(spellInfo->IsPositive() ? me : damageInfo.GetVictim(), spellInfo->Id, true, item);
//            }
//        }
//    }
//}
...
bool bot_ai::AddOrder(BotOrder&& order)
{
    ...
    {
        LOG_ERROR("scripts", "bot_ai::AddOrder: orders limit reached for %s (%u)!", me->GetName().c_str(), uint32(_orders.size()));
        return false;
    }
...
void bot_ai::CancelOrder(BotOrder const& order)
{
    if (_orders.empty())
    {
        LOG_ERROR("scripts", "bot_ai::CancelOrder: %s orders are empty while trying to remove order type %u!",
            me->GetName().c_str(), uint32(order._type));
        return;
    }
    if (_orders.front()._type != order._type || _orders.front().params.whole != order.params.whole)
    {
        LOG_ERROR("scripts", "bot_ai::CancelOrder: %s front order (type %u) is different from cur order (type %u)!",
...
void bot_ai::CompleteOrder(BotOrder const& order)
{
    if (_orders.empty())
    {
        LOG_ERROR("scripts", "bot_ai::CompleteOrder: %s orders are empty while trying to remove order type %u!",
            me->GetName().c_str(), uint32(order._type));
        return;
    }
    if (_orders.front()._type != order._type || _orders.front().params.whole != order.params.whole)
    {
        LOG_ERROR("scripts", "bot_ai::CompleteOrder: %s front order (type %u) is different from cur order (type %u)!",
            me->GetName().c_str(), uint32(_orders.front()._type), uint32(order._type));
        return;
    }
    ...
    switch (order._type)
    {
        case BOT_ORDER_SPELLCAST:
        {
            ...
            if (guid == me->GetGUID())
                ...
                target = master;
            else if (guid != ObjectGuid(uint64(0)))
            {
                if (!IAmFree())
        ...
        default:
            LOG_ERROR("scripts", "bot_ai:_ProcessOrders: invalid order type %u!", uint32(order._type));
            CancelOrder(order);
            return;
    }
}
...
void bot_ai::DoSkytalonVehicleStrats(uint32 diff)
{
    ...
    if (!drakespell)
    {
        LOG_ERROR("scripts", "DoSkytalonVehicleStrats no spell for role mask %u cp %u, power %u, target %s",
            GetBotRoles(), uint32(vehcomboPoints), drakePower, target->GetName().c_str());
        return;
    }

    //LOG_ERROR("scripts", "DoSkytalonVehicleStrats %u on %s", drakespell, target->GetName().c_str());
...
void bot_ai::DoVehicleStrats(BotVehicleStrats strat, uint32 diff)
{
    ...
    {
        //LOG_ERROR("scripts", "DoVehicleStrats doing strat %u", uint32(strat));
        curVehStrat = strat;
    }
    ...
    switch (strat)
    {
        ...
        case BOT_VEH_STRAT_GENERIC:
            ...
            break;
        default:
            LOG_ERROR("scripts", "Unhandled vehicle strat %u", uint32(strat));
            break;
    }
}
    ...
    switch (me->GetVehicleBase()->GetEntry())
    {
        ...
        */
        default:
            ...
            if (curVehStrat != strat)
                LOG_ERROR("scripts", "bot_ai DoVehicleActions: %s has to use generic strat for vehicle creature %s (%u)",
...
bool bot_ai::GlobalUpdate(uint32 diff)
{
    ...
    if (_updateTimerEx1 <= diff && !IAmFree())
    {
        ...
        */
        ...
        {
            //LOG_ERROR("spells", "bot_ai:UpdateEx by %s, found free master, my skills: %u:", me->GetName().c_str(), uint32(_roleMask & BOT_ROLE_MASK_GATHERING));
            ...
            if (!woList.empty())
            {
                ...
                if (me->GetDistance(wo) <= INTERACTION_DISTANCE * 0.5f && me->HasInArc(float(M_PI) * 0.75f, wo))
                {
                    //cosmetic
                    //CastSpellExtraArgs args(TRIGGERED_FULL_MASK);
                    me->CastSpell(wo->GetTypeId() == TYPEID_UNIT ? wo->ToUnit() : me, SPELL_COMBAT_SPECIAL_2H_ATTACK, TRIGGERED_FULL_MASK);
                ...
                else
                {
                    ...
                    {
                        Position pos;
                        wo->GetNearPoint(me, pos.m_positionX, pos.m_positionY, pos.m_positionZ, me->GetObjectSize(), CONTACT_DISTANCE, wo->GetAbsoluteAngle(me));
        ...
        if (me->IsInWorld() && HasRole(BOT_ROLE_AUTOLOOT) && HasRole(BOT_ROLE_MASK_LOOTING) &&
            ...
            !HasBotCommandState(BOT_COMMAND_MASK_UNMOVING))
        {
            //LOG_ERROR("scripts", "bot_ai Looting by %s, my mask: %u:", me->GetName().c_str(), uint32(_roleMask & BOT_ROLE_MASK_LOOTING));
            ...
            for (std::list<Unit*>::iterator itr = crList.begin(); itr != crList.end();)
            {
                ...
                {
                    //LOG_ERROR("scripts", "bot_ai Looting: cannot loot %s (%u)", (*itr)->GetName().c_str(), (*itr)->GetGUIDLow());
                    crList.erase(itr++);
                    continue;
                }
                itr++;
...
bool bot_ai::FinishTeleport(/*uint32 mapId, uint32 instanceId, float x, float y, float z, float o*/)
{
    ...
    {
        //CastSpellExtraArgs args(TRIGGERED_FULL_MASK);
        me->CastSpell(me, COSMETIC_TELEPORT_EFFECT, TRIGGERED_FULL_MASK);
    }
        ...
        master->GetVehicleBase()->IsControlledByPlayer())
    {
        ...
        if (seat && seat->CanEnterOrExit())
        {
            ...
            case CREATURE_OCULUS_DRAKE_AMBER:
            {
                ...
                veh->CastSpell(veh, 66667, TRIGGERED_NONE, nullptr, nullptr, me->GetGUID()); //Gear Scaling
                break;
            }
            //case CREATURE_TOC_STEED_QUELDOREI:
            //case CREATURE_TOC_NIGHTSABER:
...
Unit* bot_ai::SpawnVehicle(uint32 creEntry, uint32 vehEntry)
{
    ...
    TempSummon* vc = new TempSummon(nullptr, me->GetGUID(), false);
    ...
    ASSERT(vc->Create(map->GenerateLowGuid<HighGuid::Unit>(), map, master->GetPhaseMask(), creEntry, x, y, z, o, 0, nullptr));
...
void bot_ai::ChooseVehicleForEncounter(Player const* owner, uint32 &creEntry, uint32 &vehEntry) const
{
    ...
    switch (mVeh->GetBase()->GetEntry())
    {
        ...
        default:
            ...
            LOG_ERROR("scripts", "ChooseVehicleForEncounter: unhandled master vehicle creature %s (%u)",
    ...
    if (_botclass >= BOT_CLASS_EX_START)
    {
        switch (_botclass)
        {
            ...
            case BOT_CLASS_NECROMANCER:
                return BOT_CLASS_WARLOCK;
            default:
                LOG_ERROR("entities.unit", "GetPlayerClass: %s has unknown Ex bot class %u!", me->GetName().c_str(), _botclass);
                return BOT_CLASS_PALADIN;
        }
    }
    ...
    if (_botclass >= BOT_CLASS_EX_START)
    {
        switch (_botclass)
        {
            ...
            case BOT_CLASS_NECROMANCER:
                return RACE_HUMAN;
            default:
                LOG_ERROR("entities.unit", "GetPlayerRace: %s has unknown Ex bot class %u!", me->GetName().c_str(), _botclass);
                return RACE_HUMAN;
        }
    }
...
void bot_ai::InitBotCustomSpells()
{
    ...
    //sinfo->AttributesEx3 |= SPELL_ATTR3_DO_NOT_DISPLAY_RANGE;
    ...
    */
    ...
    sinfo->Effects[1].Effect = SPELL_EFFECT_NONE;
    //24) END STEAL MAGIC
    ...
    sinfo->Effects[0].TargetA = SpellImplicitTargetInfo(TARGET_UNIT_TARGET_ANY);
    sinfo->Effects[1].Effect = SPELL_EFFECT_NONE;
    sinfo->Effects[2].Effect = SPELL_EFFECT_NONE;
    //30) END RAISE DEAD
