...
uint32 bot_pet_ai::GetData(uint32 data) const
{
    switch (data)
    {
        ...
        default:
            LOG_DEBUG("entities.unit", "bot_pet_ai::GetData(): unk data type %u!", data);
            return 0;
    }
}
...
void bot_pet_ai::SetPetStats(bool force)
{
    switch (myType)
    {
        ...
        case BOT_PET_NECROSKELETON:
            break;
        default:
            LOG_ERROR("entities.player", "bot_pet_ai::SetPetStats(): unk pet type %u, aborting", myType);
            return;
    }
    ...
    if (force)
    {
        ...
        else
        {
            ...
            LOG_ERROR("entities.unit", "SetPetStats(): pInfo is NULL, setting default stats for pet %u", myType);
        }
    }
    ...
    me->HandleStatModifier(UNIT_MOD_ARMOR, BASE_VALUE, float(myarmor), true);
    me->UpdateArmor();
    //resistances
    for (uint8 i = SPELL_SCHOOL_HOLY; i != MAX_SPELL_SCHOOL; ++i)
    {
        ...
        me->HandleStatModifier(UnitMods(UNIT_MOD_RESISTANCE_START + i), BASE_VALUE, petResist, true);
    ...
    //LOG_ERROR("entities.player", "SetPetStat(): hp stamval %.1f, stammult %.1f, base %u, total %.2f", stamValue, stamMult, botPet->GetCreateHealth(), m_totalhp);
    bool fullhp = me->GetHealth() == me->GetMaxHealth();
    float pct = fullhp ? 100.f : me->GetHealthPct(); // needs for regeneration
    me->HandleStatModifier(UNIT_MOD_HEALTH, BASE_VALUE, m_totalhp, true);
    me->UpdateMaxHealth();
    ...
    if (me->getPowerType() == POWER_MANA)
    {
        ...
        //LOG_ERROR("entities.player", "SetPetStat(): mana intValue %.1f, intMult %.1f, base %u, total %.2f", intValue, intMult, botPet->GetCreatePowerValue(POWER_MANA), m_totalmana);
        ...
        me->HandleStatModifier(UNIT_MOD_MANA, BASE_VALUE, m_totalmana, true);
...
void bot_pet_ai::RefreshAura(uint32 spellId, int8 count, Unit* target) const
{
    if (count < 0 || count > 10)
    {
        LOG_ERROR("entities.player", "bot_pet_ai::RefreshAura(): count is out of bounds (%i) for bot %s (botclass: %u, entry: %u)",
            int32(count), me->GetName().c_str(), uint32(petOwner->GetBotClass()), me->GetEntry());
        return;
    }
    if (!spellId)
    {
        LOG_ERROR("entities.player", "bot_pet_ai::RefreshAura(): spellId is 0 for bot %s (botclass: %u, entry: %u)",
    ...
    if (!spellInfo)
    {
        LOG_ERROR("entities.player", "bot_pet_ai::RefreshAura(): Invalid spellInfo for spell %u! Bot - %s (botclass: %u, entry: %u)",
...
void bot_pet_ai::MoveBehind(Unit const* target) const
{
    ...
    {
        float x,y,z;
        target->GetNearPoint(me, x, y, z, me->GetObjectSize(), me->GetCombatReach(), me->GetAbsoluteAngle(target));
...
void bot_pet_ai::InitSpellMap(uint32 basespell, bool forceadd, bool forwardRank)
{
    ...
    if (!info)
    {
        LOG_ERROR("entities.player", "bot_pet_ai::InitSpellMap(): No SpellInfo found for base spell %u", basespell);
...
void bot_pet_ai::ReleaseSpellCooldown(uint32 basespell)
{
    ...
    {
        LOG_ERROR("spells", "bot_pet_ai::ReleaseSpellCooldown is called for wrong spell %u!", basespell);
        return;
    }
...
void bot_pet_ai::OnBotPetSpellInterrupted(SpellSchoolMask schoolMask, uint32 unTimeMs)
{
    ...
    for (BotPetSpellMap::iterator itr = _spells.begin(); itr != _spells.end(); ++itr)
    {
        ...
        //LOG_ERROR("entities.player", "OnBotPetSpellInterrupted(): Adding cooldown (%u, new: %u) to spell %s (id: %u, schoolmask: %u), reqSchoolMask = %u",
