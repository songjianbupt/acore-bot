...
void AuraEffect::HandleAuraModTotalThreat(AuraApplication const* aurApp, uint8 mode, bool apply) const
{
        ...
        target->ToCreature()->IsNPCBotOrPet())
    {
        ...
        if (caster && caster->IsAlive())
            caster->getHostileRefMgr().addTempThreat((float)GetAmount(), apply);
        return;
    }
    //end npcbot
            ...
            case 25155:
            {
                switch (GetId())
                {
                    ...
                    case 20810:
                    {
                        ...
                        triggerCaster->CastSpell(triggerTarget, triggeredSpellInfo, &values, TRIGGERED_FULL_MASK, nullptr, this);
                        return;
                    }
                    default:
                        break;
